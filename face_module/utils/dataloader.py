...
class LFWDataset(datasets.ImageFolder):
    def __init__(self, dir, pairs_path, image_size, transform=None, dataset="LFW"):
        super(LFWDataset, self).__init__(dir, transform)
        self.image_size = image_size
        self.pairs_path = pairs_path
        self.dataset = dataset
        if dataset == "CASIA-FaceV5":
            self.validation_images = self.get_lfw_paths(dir, file_ext="bmp")
        else:
    ...
    def get_lfw_paths(self, lfw_dir, file_ext="jpg"):
        ...
        format_string = '%04d'
        if self.dataset == 'CASIA-FaceV5':
            format_string = '%d'
        for i in range(len(pairs)):
            # for pair in pairs:
            pair = pairs[i]
            if len(pair) == 3:
                path0 = os.path.join(lfw_dir, pair[0], pair[0] + '_' + format_string % int(pair[1]) + '.' + file_ext)
                path1 = os.path.join(lfw_dir, pair[0], pair[0] + '_' + format_string % int(pair[2]) + '.' + file_ext)
                issame = True
            elif len(pair) == 4:
                path0 = os.path.join(lfw_dir, pair[0], pair[0] + '_' + format_string % int(pair[1]) + '.' + file_ext)
                path1 = os.path.join(lfw_dir, pair[2], pair[2] + '_' + format_string % int(pair[3]) + '.' + file_ext)
                issame = False
